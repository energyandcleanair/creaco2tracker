test_that(
  "front layer files do not bypass data-access boundary",
  {
...
test_that(
  "every R file belongs to exactly one configured layer",
  {
    cfg <- architecture_get_boundary_config()

    all_r_files <- basename(
      list.files(
...
test_that(
  "each configured layer has at least one file",
  {
    cfg <- architecture_get_boundary_config()
    all_r_files <- basename(
      list.files(
