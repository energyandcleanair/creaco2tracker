test_that(
  "get_demand_components with LM returns expected structure",
  {
    skip_if_offline()
...
test_that(
  "value_weather_corrected equals value for non-weather components",
  {
    skip_if_offline()
    skip_on_cran()
    ...
    if (nrow(others) > 0) {
      expect_equal(
        others$value, others$value_weather_corrected,
        tolerance = 1e-6,
    ...
    if (nrow(gas_elec) > 0) {
      expect_equal(
        gas_elec$value, gas_elec$value_weather_corrected,
        tolerance = 1e-6,
...
test_that(
  "value_weather_corrected differs from value for heating component",
  {
    skip_if_offline()
    skip_on_cran()
...
test_that(
  "both model types return same column structure",
  {
    skip_if_offline()
...
test_that(
  "heating is positive in winter months",
  {
    skip_if_offline()
