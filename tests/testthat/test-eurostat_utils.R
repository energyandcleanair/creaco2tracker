...
test_that("eurostat_split_elec_others preserves data when only 'all' sector is available", {
  ...
  expect_true(all(result$sector %in% c("electricity", "others")), info =
    "Should have electricity and others sectors")
  expect_equal(result$values[result$sector == "others"], 500, info =
    "All value should go to others when no electricity data")
  expect_equal(result$values[result$sector == "electricity"], 0, info =
