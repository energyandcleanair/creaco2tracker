...
test_that("apply_source_data_mask applies source-specific and global rules", {
  gas <- tibble(
    iso2 = c("EU", "EU", "DE"),
...
test_that("apply_source_data_mask supports source aliases and custom value_cols", {
  pwr <- tibble(
    iso2 = c("EU", "EU"),
    source = c("Wind", "Solar"),
...
test_that("get_data_masking_config returns expected top-level keys", {
...
test_that("granular entsog masking supports flow-type filters", {
  entsog_raw <- tibble(
...
test_that("granular eurostat monthly masking uses time column", {
  eurostat_gas <- tibble(
    iso2 = c("EU", "EU"),
...
test_that("fill_weather does not forward fill across explicitly masked rows", {
  weather_raw <- tibble(
...
test_that("fill_weather still fills inserted gaps when no explicit NA is present", {
  weather_raw <- tibble(
...
test_that("get_gas_demand_consdist preserves explicitly masked gaps", {
  entsog_data <- tibble(
    destination_iso2 = c("DE", "DE", "DE"),
    departure_iso2 = c("NL", "NL", "NL"),
    date = as.Date(c("2024-01-01", "2024-01-02", "2024-01-03")),
...
test_that("get_gas_demand_apparent preserves explicitly masked gaps", {
  entsog_data <- tibble(
    destination_iso2 = c("DE", "DE", "DE"),
    departure_iso2 = c("NL", "NL", "NL"),
    date = as.Date(c("2024-01-01", "2024-01-02", "2024-01-03")),
