...
test_that(
  "apply_seasonal_adjustment works correctly with valid data",
  {
    ...
    jan_values <- result %>%
      filter(lubridate::month(time) == 1) %>%
      pull(values)
    dec_values <- result %>%
      filter(lubridate::month(time) == 12) %>%
      pull(values)
...
test_that(
  "apply_seasonal_adjustment handles missing values correctly",
  {
...
test_that(
  "apply_seasonal_adjustment handles edge cases",
  {
