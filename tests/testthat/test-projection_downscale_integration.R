...
create_growing_trend_data <- function(
  value_init = 0, add_per_period = 100,
...
create_pwr_generation_mock <- function(
  value_init = 0, add_daily = 10,
  date_min = as.Date("2020-01-01"), date_max =
    as.Date("2020-07-15"),
...
create_gas_demand_mock <- function(
  value_init = 0, add_daily = 2,
  date_min = as.Date("2020-01-01"), date_max =
    as.Date("2020-07-15"),
...
create_eurostat_cons_mock <- function(
  value_init = 0, add_daily = 100,
  date_min = as.Date("2020-01-01"), date_max =
    as.Date("2020-05-01"),
  fuels = c("coal", "gas"), sectors = c("electricity", "all")
) {
...
test_that(
  "Projection and downscaling work well together",
  {
    ...
    ggplot(eurostat_cons %>% filter(sector == "electricity"), aes(
      x = time, y = values, color =
        fuel
    )) +
      geom_line() +
      geom_point()
    ...
    co2_daily <- downscale_daily(
      co2_monthly,
      pwr_generation = pwr_generation, gas_demand =
    ...
    expect_true(
      max(
        abs(
          deltas$delta_sd[deltas$fuel != FUEL_COAL |
            deltas$sector != SECTOR_ALL]
        )
      ) < 0.1
    )
  }
