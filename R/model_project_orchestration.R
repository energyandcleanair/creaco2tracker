...
project_until_now <- function(
  ...
  dts_month <- seq.Date(min(co2_unprojected$date), if (is.null(date_to)) today() %>%
    "day<-"(1) else as.Date(date_to), by = "month")

  co2_unprojected %>%
    split_gas_to_elec_all() %>%
    project_until_now_elec(pwr_generation = pwr_generation, dts_month = dts_month, fill_mode =
      fill_mode) %>%
