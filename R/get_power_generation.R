...
#
# Pipeline (per iso2 × Ember source):
...
get_power_generation <- function(iso2s = get_eu_iso2s(include_eu = TRUE),
  ...
  # Need long history for ETS even if the caller asks for a narrower window.
  fetch_from <- min(date_from, as.Date("2018-01-01"))

  message("Fetching ENTSOE daily...")
  ...
  message("Fetching EMBER monthly...")
  ember_monthly <- ember.get_power_generation(frequency = "monthly", iso2s = iso2s,
                                              use_cache = use_cache)
  message("Fetching EMBER yearly...")
  ember_yearly  <- ember.get_power_generation(frequency = "yearly",  iso2s = iso2s,
                                              use_cache = use_cache)

  # Country / region metadata to re-attach to the scaled output.
  country_info <- entsoe_daily %>% select(iso2, region, country) %>% distinct()
...
.aggregate_entsoe_monthly <- function(entsoe_daily) {
  entsoe_daily %>%
    ...
    # source × country × date triple even when the source is absent.
    filter(n_days > 0) %>%
    mutate(
...
.extend_ratio <- function(df, target_date, frequency = 12L, min_obs = 12L,
  ...
  res <- pbapply::pblapply(groups, function(g) {
    ...
    full %>% mutate(iso2 = iso2_v, source = source_v) %>%
...
.distribute_ember_monthly_to_daily <- function(ember_monthly, keys, date_from, date_to, country_info) {
  ...
  if (nrow(ember_subset) == 0) return(NULL)
  ...
  extension <- last_values %>%
    ...
    group_modify(function(df, .) {
      if (df$last_date >= end_month) return(tibble())
...
.generate_power_diagnostics <- function(entsoe_monthly,
  ...
  build_forecast_with_anchor <- function(df) {
    df %>%
      ...
      group_modify(function(g, .) {
        ...
        if (nrow(fcst) == 0) return(tibble())
        anchor_pool <- g %>% filter(type %in% c("observed", "interpolated"))
        if (nrow(anchor_pool) == 0) return(fcst)
  ...
  for (src in sources) {
    ...
    p <- df_src %>%
      ...
      geom_line(data = filter(df_src, type %in% c("observed", "interpolated")),
                aes(y = ratio_used), color = "#2166AC", linewidth = 0.5) +
      # Forecast segment with anchor point
      geom_line(data = fcst_segment,
                aes(y = ratio_used), color = "#D6604D", linewidth = 0.7) +
      # Mark interpolated months
      geom_point(data = filter(df_src, type == "interpolated"),
                 aes(y = ratio_used), color = "#FDB863", size = 0.9) +
      # Mark forecast months
      geom_point(data = filter(df_src, type == "forecast"),
                 aes(y = ratio_used), color = "#D6604D", size = 0.9) +
      # Outliers (× on the original observed ratio)
      geom_point(data = filter(df_src, is_outlier),
  ...
  post_monthly <- scaled_monthly %>%
    ...
    mutate(ratio_y    = coalesce(ratio_y, 1),
  ...
  for (src in sources) {
    ...
    p <- df_src %>%
      ...
      labs(title    = glue("Post-monthly residual - {src}"),
           subtitle = paste0(
  ...
  for (src in sources) {
    ...
    p <- df_src %>%
      ...
      facet_wrap(~iso2, scales = "free_y", ncol = 7) +
      labs(title    = glue("Yearly ratio (EMBER_yearly / sum(EMBER_monthly)) - {src}"),
           subtitle = "Blue = observed + interpolated. Red = forecast. Orange dots = interpolated. Red x = outlier.",
           x = NULL, y = "Ratio") +
      theme_minimal(base_size = 9) +
      theme(strip.text  = element_text(size = 8, face = "bold"))
  ...
  for (src in sources) {
    ...
    p <- df_src %>%
      ...
      facet_wrap(~iso2, ncol = 7) +
      coord_cartesian(ylim = c(-100, 100)) +
      labs(title    = glue("Post-yearly residual - {src}"),
           subtitle = paste0(
  ...
  if (!is.na(latest_year)) {
    ...
    if (nrow(combined_factor) > 0) {
      p_heat <- combined_factor %>%
        ...
        labs(title    = glue("Combined ENTSOE -> Ember scale factor ({year(latest_year)})"),
             subtitle = "Geometric-mean monthly factor × yearly factor",
             x = "Source", y = "Country") +
        theme_minimal() +
        theme(axis.text.y = element_text(size = 8))
      ggsave(file.path(diagnostics_folder, "final_scale_heatmap.png"),
             p_heat, width = 8, height = 12)
    }
  }
  ...
  for (src in sources) {
    df <- entsoe_for_plot %>% filter(source == src) %>%
      ...
      full_join(scaled_monthly        %>% filter(source == src) %>%
    ...
    p <- ggplot(df, aes(x = date, y = value_twh, color = series,
      ...
      labs(title    = glue("Monthly generation: ENTSOE vs Ember vs scaled - {src}"),
           subtitle = paste(
