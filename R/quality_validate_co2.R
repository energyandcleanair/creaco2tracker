...
validate_co2_historical <- function(
  co2 = NULL,
  validation_data,
  iso2s = NULL,
  folder = "diagnostics",
  ...
  if (all_countries) {
    ...
    colors <- unname(
      rcrea::pal_crea[c(
        "Blue", "Dark.red", "Dark.blue", "Orange", "Red",
    ...
    local({
      plt_from_data <- function(plt_data) {
        facet_layer <- if (length(unique(co2_crea$iso2)) > 1) {
          facet_wrap(
            ~ iso2_to_name(iso2),
            scales = "free_y",
            ncol = 4
          ...
          labs(
            title =
              glue(
                ...
                "{if(exclude_international_aviation) ' (without international aviation)' else ''}"
              ),
            ...
            alpha = "Source",
            color = "Source",
            caption = "Source: CREA analysis and Global Carbon Budget 2025 (
    ...
    lapply(iso2s, function(iso2) {
      ...
      plt <- ggplot(plot_data) +
        geom_line(
          aes(
            year, value / 1e3,
            col = source, linewidth = source, alpha = source,
            linetype = type
          )
        ) +
      ...
      if ("EU" %in% iso2s) {
        ...
        interpolated <- validation_data %>%
          ...
          left_join(
            co2_extended %>%
              ...
              select(iso2, target_value = value, source)
          ) %>%
          dplyr::summarise(
            interpolated_year = {
        ...
        plt_full <- plt +
          ...
          ggrepel::geom_text_repel(
            data = interpolated,
            aes(
              x = interpolated_year, y = 4, label = round(interpolated_year), col = source,
              alpha = "CREA"
            ),
            size = 4,
            show.legend = FALSE,
      ...
      (
        plt <- plt_data %>%
          ...
          labs(
            ...
            subtitle = paste0(
              "Million tonne CO2 per day", if_else(
                is_daily, "30-day moving average",
                ""
              )
            ),
...
validate_co2_monthly <- function(co2, folder = "diagnostics") {
  ...
  co2_crea <- co2 %>%
    ...
    summarise(
      value = sum(value) / 1e6 / (as.integer(difftime(max(date), min(date))) + 1),
      unit = "mt/day",
  ...
  co2_validate <- bind_rows(
    ...
    co2_carbonmonitor %>%
      ...
      summarise(
        value = sum(value * case_when(country == "United Kingdom" ~ -1, TRUE ~ 1)),
        country = "EU27",
        iso2 = "EU"
      )
  ) %>%
    ...
    summarise(
      value = sum(value / lubridate::days_in_month(date)),
      unit = "mt/day",
      .groups = "drop"
    ) %>%
    ungroup()
  ...
  ggplot(
    ...
    scale_color_manual(
      values = unname(
        rcrea::pal_crea[c(
          "Blue", "Dark.red", "Orange", "Dark.blue",
          "Purple", "Green", "Red"
        )]
      )
    ) +
    ...
    rcrea::scale_y_crea_zero() +
    labs(
      title = "EU CO2 emissions from fossil fuels",
...
validate_co2_transport <- function(
  co2,
  folder = "diagnostics",
  date_from = "1990-01-01",
  year_to = NULL
  ...
  eea_w_international <- read_csv(filepath) %>%
    ...
    filter(
      .data[["Pollutant_name"]] == "CO2",
      .data[["Unit"]] == "Gg"
    ) %>%
    group_by(year = as.numeric(.data[["Year"]]), iso2 = "EU") %>%
  ...
  lapply(iso2s, function(iso2) {
    ...
    colors <- unname(
      rcrea::pal_crea[c(
        "Blue", "Dark.red", "Dark.blue", "Orange", "Red", "Yellow",
        "Dark.violet", "Turquoise"
      )]
    )

    facet_layer <- if (length(unique(plot_data$iso2)) > 1) {
      facet_wrap(~iso2, scales = "free_y")
    } else {
      NULL
    }
    ...
    plt <- ggplot(plot_data) +
      ...
      labs(
        ...
        alpha = "Source",
        color = "Source",
        caption = "Source: CREA analysis based on EUROSTAT and IPCC."
      )
...
compare_co2_versions <- function(
  ...
  plt <- co2 %>%
    ...
    labs(
      title = "Comparison with previous version",
      subtitle = "Year-on-year changes of EU CO2 emissions",
      y = NULL, x = NULL, fill = "Version"
    )
