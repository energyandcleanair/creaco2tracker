...
update_co2_daily <- function(
  production,
  diagnostics_folder,
  data_masking = DATA_MASKING_NONE
) {
  co2_daily <- get_co2(
    diagnostics_folder = diagnostics_folder, downscale_daily = TRUE, iso2s = "EU",
    data_masking = data_masking
  )
  upload_co2_daily(co2_daily, production = production)
}

update_corrected_demand <- function(
  production,
  diagnostics_folder,
  data_masking = DATA_MASKING_NONE
) {
...
update_all <- function(
  ...
  update_co2_daily(
    production = production, diagnostics_folder = diagnostics_folder, data_masking = data_masking
  )
