...
update_co2_daily <- function(production, diagnostics_folder, data_masking = NULL) {
  co2_daily <- get_co2(diagnostics_folder = diagnostics_folder, downscale_daily = T, iso2s = "EU", data_masking = data_masking)
...
update_all <- function(production = T, diagnostics_folder = "diagnostics", data_masking = NULL) {
  library(tidyverse)
  library(lubridate)
  library(tidytext)
  library(countrycode)
  ...
  update_gas_demand(production = production, diagnostics_folder = diagnostics_folder, data_masking = data_masking)
  update_co2_daily(production = production, diagnostics_folder = diagnostics_folder, data_masking = data_masking)
  update_corrected_demand(production = production, diagnostics_folder = diagnostics_folder, data_masking = data_masking)
