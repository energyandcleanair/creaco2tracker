diagnostic_pwr <- function(pwr_generation, diagnostics_folder = "diagnostics") {
  ...
  if (!is_null_or_empty(diagnostics_folder)) {
    ...
    plt_range <- plt_data %>%
      ...
      summarise(
        min = min(output_mw_rollmean), max = max(output_mw_rollmean),
        .groups = "drop"
      )
    ...
    plt <- plt_data %>%
      ...
      geom_ribbon(
        data = plt_range %>% filter(country == "EU total"),
      ...
      labs(
        title = "EU power generation by source",
...
diagnostic_eurostat_cons_yearly_monthly <- function(
  ...
  if (!is_null_or_empty(diagnostics_folder)) {
    ...
    ggsave(
      filename = file.path(diagnostics_folder, "eurostat_annual_vs_monthly_yearly.png"),
      plot = plt,
      width = 12,
      height = 18,
      bg = "white",
...
diagnostic_eurostat_cons <- function(
  ...
  if (!is_null_or_empty(diagnostics_folder)) {
    ...
    plt <- ggplot(plt_data, aes(time, values, col = sector)) +
      geom_line() +
      rcrea::scale_y_crea_zero() +
    ...
    ggsave(
      filename = file.path(diagnostics_folder, "eurostat_cons.png"),
      width = 10,
    ...
    plt <- eurostat_cons %>%
      ...
      facet_wrap(fuel ~ sector)

    plt
    quicksave(
      file = file.path(diagnostics_folder, "eurostat_cons_availability.png"),
      width = 10,
      height = 8,
      bg = "white",
      plot = plt,
