...
- If a table or plot is missing, report that as an incomplete artifact unless `metadata.csv` and
  `complete.ok` show the report was intentionally generated by an older workflow.
...
- Remember that the current working-copy driver, collection script, and report script define cache
  identity, even when those scripts are uncommitted or untracked.
