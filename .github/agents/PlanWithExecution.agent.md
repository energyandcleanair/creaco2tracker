...
- You MAY use terminal execution for discovery evidence only via #tool:execute/runInTerminal (for example: list files, inspect git status, run read-only checks, or run bounded test commands to gather facts).
- #tool:execute/runInTerminal is STRICTLY read-only in this agent. NEVER use it to modify, generate, or write code or files.
- Keep terminal usage safe and bounded: prefer short, deterministic commands; avoid destructive commands; if a command could change files, do not run it.
...
When codebase signals are incomplete, run bounded terminal commands with #tool:execute/runInTerminal to gather additional evidence. Use #tool:execute/getTerminalOutput when a command continues in the background or requires follow-up output checks.
